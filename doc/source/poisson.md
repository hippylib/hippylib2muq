...
which is implemented by `BiLaplacianPrior` class that provides methods to apply the regularization (precision) operator to a vector or to apply the prior covariance operator.
...
The class `PointwiseStateObservation` implements the evaluation of the
